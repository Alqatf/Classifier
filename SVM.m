function SVMOpt = SVM(fea_train, gnd_train)
...
for i = 1:8
    for k = 1:8
        %{
        SVMModel = fitcsvm(fea_train,gnd_train,...
                         'BoxConstraint',c(k),'KernelFunction','rbf',...
                     'Crossval', 'on','kFold',5,'KernelScale',gamma(i));
        %CVSVMModel = crossval(SVMModel);
        classLoss = kfoldLoss(SVMModel);
        cvCorr = 1 - classLoss;
        SVMOpt = [SVMOpt; [c(k) gamma(i) cvCorr]];
        %}
